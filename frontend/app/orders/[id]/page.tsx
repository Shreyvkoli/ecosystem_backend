                ...
                <div className="bg-black aspect-video flex flex-col items-center justify-center text-white rounded-lg p-6 text-center">
                  {user.role === 'EDITOR' && order.editorDepositRequired && order.editorDepositStatus !== 'PAID' ? (
                    <div className="max-w-md space-y-4">
                      <svg className="w-12 h-12 mx-auto text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                      <h3 className="text-xl font-bold">Raw Files Locked</h3>
                      <p className="text-gray-300">
                        The creator has assigned this job to you. To access the raw files and start working, please pay the fully refundable security deposit.
                      </p>
                      <div className="pt-2">
                        {/* Button wrapped in div to avoid hydration issues if necessary, but component handles it */}
                        {/* Accessing EditorDepositButton via the sidebar usually, but adding a visual cue here also helps */}
                      </div>
                    </div>
                  ) : (
